...
func runClientCommand(command string) {
	...
	// 获取客户端标志
	serverURL, _ := clientCmd.PersistentFlags().GetString("server")
	outputFormat, _ := clientCmd.PersistentFlags().GetString("output")
	timeout, _ := clientCmd.PersistentFlags().GetInt("timeout")
