...
| 4 | Controller | `ShouldBindJSON(&DTO)` 参数校验 → 调用 Service | `internal/controller/*.go` |
| 5 | Service | 事务控制、聚合多 Biz、调用队列 | `internal/service/*.go` |
| 6 | Biz | 领域规则、状态机、计算 | `internal/biz/*` |
| 7 | Repository | GORM DAO，读写 DB | `internal/data/repository/*.go` |
| 8 | External | PostgreSQL / Redis / Kafka | Docker / Cloud |
...
| 5 | Controller | 封装统一 `Response{code,msg,data}` |
| 6 | Middleware | Trace 记录、错误转换、Header 填充 |
| 7 | Router | 交由 Gin 写 Socket |
| 8 | Client | 接收 JSON，渲染 UI |
...
| # | 文件 | 操作 |
|---|------|------|
| 1 | `docs/request-response-two-perspectives.md` | 新增本文档 |
