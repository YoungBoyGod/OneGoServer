...
### 1.1 功能概述
队列注册功能用于在系统中注册新的队列资源，支持多种队列类型（如普通队列、优先级队列、延迟队列等），为后续任务分发、消息传递等提供基础。
...
**响应结构：**
```go
type QueueRegisterResponse struct {
    Code    int    `json:"code"`
...
#### 4.2.1 队列注册服务接口
```go
type QueueRegisterService interface {
    RegisterQueue(ctx context.Context, req *QueueRegisterRequest) (*QueueRegisterResult, error)
}
```

#### 4.2.2 队列注册仓储接口
```go
type QueueRegisterRepository interface {
...
CREATE TABLE queue (
    ...
    queue_name VARCHAR(64) NOT NULL UNIQUE COMMENT '队列名称',
    queue_type VARCHAR(20) NOT NULL COMMENT '队列类型',
    capacity INT DEFAULT 0 COMMENT '容量',
    attributes JSON COMMENT '自定义属性',
...
type QueueRegisterRequest struct {
...
type QueueRegisterResult struct {
    QueueID   string `json:"queue_id"`
    QueueName string `json:"queue_name"`
    QueueType string `json:"queue_type"`
    CreatedAt string `json:"created_at"`
...
sequenceDiagram
    ...
    participant Service as 队列注册服务
    participant Repository as 数据仓储
    participant DB as 数据库
    
    Client->>API: POST /api/v1/queue/register
    API->>Controller: 路由到队列注册控制器
